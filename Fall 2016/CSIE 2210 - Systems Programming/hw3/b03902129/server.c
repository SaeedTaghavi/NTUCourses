...
int main(int argc, char **argv) {
    ...
    init_http_server(&server, (unsigned short) atoi(argv[1]));    // Initialize http server
    ...
    while (1) {                             /* Main loop */
        ...
        if (select(128, &read_fds, NULL, NULL, NULL) == -1)
            ERR_EXIT("select")
            ...
            if (FD_ISSET(i, &read_fds)) {
                if (i == server.listen_fd) {
                    ...
                    if (ret == 1) continue;
                        ...
                        switch (err) {
                            case 1: {
                                static char *msg = "HTTP/1.1 401 Unauthorized\n""Content-type: text/html; charset=iso-8859-1\n""\n""<html>\n"" <body>\n""  <h1>Unauthorized</h1>\n""  <p>client connection error</p>\n"" </body>\n""</html>\n";
                                write(requestP[i].conn_fd, msg, strlen(msg)); break; }
                            case 2: { 
                                static char *msg = "HTTP/1.1 400 Bad Request\n""Content-type: text/html; charset=iso-8859-1\n""\n""<html>\n"" <body>\n""  <h1>Bad Request</h1>\n""  <p>bad request, cannot parse request</p>\n"" </body>\n""</html>\n";
                                write(requestP[i].conn_fd, msg, strlen(msg)); break; }
                            case 3: { 
                                static char* msg = "HTTP/1.1 405 Method Not Allowed\n""Content-type: text/html; charset=iso-8859-1\n""\n""<html>\n"" <body>\n""  <h1>Method Not Allowed</h1>\n""  <p>method not implemented</p>\n"" </body>\n""</html>\n";;
                                write(requestP[i].conn_fd, msg, strlen(msg)); break; }
                            case 4: {
                            ...
                            case 5: { 
                                ...
                                write(requestP[i].conn_fd, msg, strlen(msg)); break; }
                            case 6: {
                                static char *msg = "HTTP/1.1 404 Not Found\n""Content-type: text/html; charset=iso-8859-1\n""\n""<html>\n"" <body>\n""  <h1>Not Found</h1>\n""  <p>file not found</p>\n"" </body>\n""</html>\n";
                                write(requestP[i].conn_fd, msg, strlen(msg)); break; }
                            case 7: {
                                static char *msg = "HTTP/1.1 400 Bad Request\n""Content-type: text/html; charset=iso-8859-1\n""\n""<html>\n"" <body>\n""  <h1>Bad Request</h1>\n""  <p>file is protected</p>\n"" </body>\n""</html>\n";
                                write(requestP[i].conn_fd, msg, strlen(msg)); break; }
                            default: 
                                write(requestP[i].conn_fd, "???", 3); break; 
                        }
                        fprintf(stderr, "=============================================\n");
                        ...
                        if (!strncmp(requestP[i].file, "info", 4)) {
                            ...
                            if ((pid_info = fork()) < 0) 
                                ...
                                // sleep(2);              /* Without sleeping, there is some select() error */
                                wait(NULL);
                            }
                            else {                           /* child */
                            ...
                            nwritten = write(requestP[i].conn_fd, info, strlen(info));      /* Write the info to client */
                            ...
                            if (pipe(fd1) == -1)
                                ERR_EXIT("pipe error: fd1")
                            if (pipe(fd2) == -1)
                                ERR_EXIT("pipe error: fd2")
                            ...
                            /* Init Map */
                            if ((mapfd = open("info.txt", O_RDWR | O_TRUNC | O_CREAT, 0777)) < 0)
                                ERR_EXIT("open")
                            lseek(mapfd, sizeof(MapInfo), SEEK_SET);
                            write(mapfd, "", 1);
                            map = (MapInfo*) mmap(0, sizeof(MapInfo), PROT_READ | PROT_WRITE, MAP_SHARED, mapfd, 0);
                            close(mapfd);
                            ...
                            if ((pid = fork()) < 0) 
                                ...
                                if (write(fd1[1], requestP[i].query, sizeof(requestP[i].query)) < 0)    /* Parent write to fd1[1] */
                                    ERR_EXIT("write error to pipe")

                                char recv[1024] = "";
                                if (read(fd2[0], recv, sizeof(recv)) < 0)                               /* Parent read from fd2[0] */
                                    ERR_EXIT("read error from pipe")

                                nwritten = write(requestP[i].conn_fd, recv, strlen(recv));
                                fprintf(stderr, "complete writing %d bytes on fd %d\n", nwritten, requestP[i].conn_fd);
                                fprintf(stderr, "=============================================\n");
                                ...
                                if (execl(requestP[i].file, requestP[i].file, (char *)0) < 0)
...
static int read_header_and_file(http_request* reqP, int *errP) {
    ...
    if ((fp = stat(reqP->file, &file_stat)) < 0)              
        ERR_RET(6)
    ...
    if (!strncmp(query, "filename=", 9)) {                      
        ...
        add_to_buf(reqP, buf, buflen);
        buflen = snprintf(buf, sizeof(buf), "Content-Length: %lld\015\012", (int64_t) sb.st_size);
        add_to_buf(reqP, buf, buflen);
    ...
    fprintf(stderr, "(conn_fd, status, file, query, host) = (%d, %d, %s, %s, %s)\n", reqP->conn_fd, reqP->status, reqP->file, reqP->query, reqP->host);
...
static void sig_handler(int signo) {
    ...
    if (signo == SIGUSR1) {
        ...
        wait(NULL);
        sprintf(info, "%d processes died previously.\nPIDs of Running Processes:", died);
