    ...
    while (1) {
        ...
        if (player[0] == -1) {
            ...
            for (int i = 0; i < 4; i++) {
                if ((pid[i] = fork()) < 0)
                    error("ERROR fork()\n");
                if (pid[i] == 0 && i == 0) execl("./player", "./player", argv[1], "A", random_key[0], (char*) 0);
                if (pid[i] == 0 && i == 1) execl("./player", "./player", argv[1], "B", random_key[1], (char*) 0);
                if (pid[i] == 0 && i == 2) execl("./player", "./player", argv[1], "C", random_key[2], (char*) 0);
                if (pid[i] == 0 && i == 3) execl("./player", "./player", argv[1], "D", random_key[3], (char*) 0);
            }
            ...
            for (int i = 0; i < 20; i++) {
                for (int j = 0; j < 4; j++) {
                    fscanf(fp[4], "%s%d%d", player_id, &key, &number_choose);     /* judge read from 4 players via judge[num].FIFO*/
                    fflush(fp[4]);
                    if (number_choose != 0) {
            ...
            sprintf(buf, "%d %d\n%d %d\n%d %d\n%d %d\n", player[0], score[0], player[1], score[1], player[2], score[2], player[3], score[3]);
            printf("%s", buf);
            fflush(stdout);
