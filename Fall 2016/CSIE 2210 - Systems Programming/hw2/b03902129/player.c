...
int main (int argc, char *argv[]){
	...
	for (int i = 0; i < 20; i++) {
		if (i == 0) {												/* first send */
			if (strcmp("A", argv[2]) == 0) fprintf(fp[1], "%s %s %d\n", "A", argv[3], a);
			else if (strcmp("B", argv[2]) == 0) fprintf(fp[1], "%s %s %d\n", "B", argv[3], b);
			else if (strcmp("C", argv[2]) == 0) fprintf(fp[1], "%s %s %d\n", "C", argv[3], c);
			else if (strcmp("D", argv[2]) == 0) fprintf(fp[1], "%s %s %d\n", "D", argv[3], d);
			fflush(fp[1]);
		}
		else {
