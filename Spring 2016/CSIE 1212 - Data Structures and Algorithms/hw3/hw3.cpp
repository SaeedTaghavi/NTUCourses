    ...
    if (k == 0) {
        ...
        if (zeros == 0) {
            if (getNum)
                ...
                if ROW {
                    if (newFirst) cmp2(maze[idx], trans(data[idx], blank, runs[idx], idx), idx);
                    else cmp2(realTmp ,trans(data[idx], blank, runs[idx], idx), idx);
                }
                else if COL {
                    if (newFirst) cmp2(rowMaze(maze, idx), trans(data[idx], blank, runs[idx], idx), idx);
                    else cmp2(realTmp ,trans(data[idx], blank, runs[idx], idx), idx);
                }
            }
            else {
                if (first) {
                    ...
                    for (int i = 0; i < t; i++)
                        firstTmp[i] = childTmp[i];
                    cmp(firstTmp, trans(data[idx], blank, runs[idx], idx), idx);
                } else cmp(firstTmp, trans(data[idx], blank, runs[idx], idx), idx);
            }
        }
    }
    else {
...
void Nonogram::cmp2(int* a ,int* b, int idx) {
    ...
    if (newFirst) {
        ...
        realBase = new bool[t];
        for (int i = 0; i < t; i++)
            realBase[i] = newBase[i];
    }
    else {
        if ROW
            secBase = newBool(maze[idx], b, idx);
        else if COL
            secBase = newBool(rowMaze(maze, idx), b, idx);
        bool allSame = true;
        for (int i = 0; i < t; i++)
...
int *Nonogram::trans(int* datas, int* zeros, int runs, int idx) {
    ...
    int i = 0;
    for (int cnt = zeros[0]; cnt > 0; cnt--, i++)
        tmp[i] = 0;
    
    for (int j = 0; j < runs; j++) {
        for (int x = datas[j]; x > 0; x--, i++)
            tmp[i] = 1;
        if (j < runs - 1) {
            tmp[i] = 0;
            i++;
        }
        for (int y = zeros[j + 1]; y > 0; y--, i++)
            tmp[i] = 0;
    }
    return tmp;
}
...
void Nonogram::readData(int m, int n) {
    ...
    for (int i = 0; i < m + n; i++) {
        ...
        for (int bufPos = 0; bufPos < buf.size(); bufPos++)
            if (buf[bufPos] == ' ') {
                if (bufPos - tmp == 2)
                    ...
                    data[i][j] = 10 * (buf[bufPos - 2] - '0') + buf[bufPos - 1] - '0';
                runs[i]++;
                j++;
                tmp = bufPos;
            }
        if (buf.size() - tmp == 2)
            ...
            data[i][j] = 10 * (buf[buf.size() - 2] - '0') + buf[buf.size() - 1] - '0';
        runs[i]++;
    }
}
