...
public:
    ...
    void swap(const Pos& a, const Pos& b)   { Node s = *a; *a = *b; *b = s; }
    void removeMin();
...
void VectorSellTree::removeMin() {
    if (size() == 1)
        ...
        while (hasLeft(u)) {
            Pos v = left(u);
            if (hasRight(u) && (isLess(right(u), v) || (v->price == right(u)->price && v->bidId > right(u)->bidId)))
                v = right(u);
...
void VectorSellTree::insert(const Node& s) {
    ...
    while (!isRoot(v)) {
        Pos u = parent(v);
        if (isLess(u, v) || (u->price == v->price && u->bidId < v->bidId)) break;
        swap(v, u);
        v = u;
    }
}
...
void VectorBuyTree::removeMax() {
    if (size() == 1)
        ...
        while (hasLeft(u)) {
            Pos v = left(u);
            if (hasRight(u) && (isLess(v, right(u)) || (v->price == right(u)->price && v->bidId > right(u)->bidId)))
                v = right(u);
...
void VectorBuyTree::insert(const Node& b) {
    ...
    while (!isRoot(v)) {
        Pos u = parent(v);
        if (isLess(v, u) || (u->price == v->price && u->bidId < v->bidId)) break;
        swap(u, v);
        v = u;
    }
}
...
int main() {
    ...
    while (scanf("%d%d%d%d%d", &bidId, &clientId, &action, &price, &shareCount) == 5) {
        if (action == 0) { //buy action
            if (ST.size()) {
                while (price >= ST.root()->price && ST.size() && shareCount) {
                    ...
                    printf("%d\t%d\t%d\t%d\t", transId, clientId, ST.root()->clientId, ST.root()->price);
            ...
            if (BT.size()) {
                while (price <= BT.root()->price && BT.size() && shareCount) {
                    ...
                    printf("%d\t%d\t%d\t%d\t", transId, BT.root()->clientId, clientId, price);
